...
## Скрипты настройки окружения для разработки

Добавлены вспомогательные скрипты для автоматизации поднятия зависимостей и применения схемы БД.
...
- PowerShell: `scripts/setup-dev.ps1`
  ...
  ```powershell
  # Устанавливает зависимости, запускает контейнеры, применяет схему и запускает dev-сервер
  .\scripts\setup-dev.ps1
  ```

- Кроссплатформенный Node-скрипт: `scripts/setup-dev.js`

  Пример использования (любая оболочка с Node.js):

  ```bash
  node ./scripts/setup-dev.js
  ```

  Node-скрипт делает те же шаги, но работает на Windows, macOS и Linux — использует локальные `docker` и `npm` и стартует dev-сервер в фоновом режиме.
...
- **Зависимости:**  
  После обновления кода выполните `npm install`, чтобы установить новые пакеты (`@vitest/coverage-v8`, `c8`, Husky, lint-staged).

- **Окружение:**  
  Проверьте `.env` — новые переменные могут появиться.  
  Для интеграционных тестов и разработки должны быть запущены Redis и PostgreSQL.

- **Тестирование:**  
  Интеграционные тесты теперь автоматически стартуют Fastify-сервер.  
  Redis и Postgres должны быть доступны локально (или через Docker).  
  Для полной изоляции можно использовать Testcontainers (см. TODO).

- **Безопасность и автоматизация:**  
  - Dependabot и CodeQL включены для автоматических проверок безопасности.
  - Линтинг, форматирование и покрытие тестами теперь проверяются в CI.

### Шаги миграции

1. Получите последние изменения.
2. Выполните `npm install`.
3. (Опционально) Пересоздайте `.env` из `.env.example`, если появились новые переменные.
4. Запустите зависимости: `docker compose up -d`
5. Примените схему:  
   `Get-Content .\schema.sql | docker compose exec -T postgres psql -U postgres -d skinport`
6. Запустите тесты:  
   `npm run test:coverage`
7. Проверьте статус CI и оповещения безопасности в GitHub.
