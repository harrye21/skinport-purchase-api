...
let dbMock: { client: { begin: ReturnType<typeof vi.fn> }; tx: ReturnType<typeof vi.fn> } | null = null;
...
const createDbMock = ({ startingBalance, price }: { startingBalance: number; price: number }) => {
  ...
  return {
    client: {
      begin: vi.fn(async (callback: (tag: typeof tx) => Promise<unknown>) => callback(tx))
    },
    tx
  };
};
